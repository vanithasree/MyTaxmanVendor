...
class GApiResponse {
    ...
    class SearchSurbs {
        ...
        class func initWithData(_ data:[String:Any]) -> SearchSurbs {
            let object = SearchSurbs()
            object.postalCode = data["postal_code"] as? String ?? ""
            if let pId = data["place_id"] as? String,let address = data["formatted_address"] as? String {
...
class GoogleApi : NSObject {
    ...
    enum UsedFor {
        ...
        //case searchSuburbPostalAndState
    }
    ...
    //https://maps.googleapis.com/maps/api/place/autocomplete/json?input=s&key=AIzaSyBXugsG-nxQoQMEbIUAEaspFtS7-4rnOFk&components=country:NSW, NSLocalizedDescription=cancelled, NSErrorFailingURLKey=https://maps.googleapis.com/maps/api/place/autocomplete/json?input=s&key=AIzaSyBXugsG-nxQoQMEbIUAEaspFtS7-4rnOFk&components=country:nsw
    ...
    enum GoogleUrl : String {
       ...
       // case searchSuburbPostalAndState = "https://maps.googleapis.com/maps/api/place/autocomplete/json"
    }
    ...
    func getUrl(_  usedFor:UsedFor,input:String) -> String {
        ...
        case .autocomplete:
            ...
            return String(format: "%@?input=%@&key=%@&components=country:au",GoogleUrl.autocomplete.rawValue,updatedInput,googleApiKey)
        case .placeInformation:
        ...
        case .nearBy:
            ...
            //        case .searchSuburbPostalAndState:
            //            let updatedInput = input.lowercased()
            //            return String(format: "%@?input=%@&key=%@&components=country:au&types=postal_code",GoogleUrl.autocomplete.rawValue,updatedInput,googleApiKey)
        }
    }
